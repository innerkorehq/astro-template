import type { FaqBlockData, FaqAppearance, FaqLink, FaqItem, FaqMedia, FaqIcon } from "./types";

function text(value: unknown): string | undefined {
  return typeof value === "string" && value.trim() ? value : undefined;
}

function media(value: unknown): FaqMedia | undefined {
  if (typeof value === "string") return { src: value, alt: "" };
  if (typeof value === "object" && value !== null && "src" in value) {
    return value as FaqMedia;
  }
  return undefined;
}

export function resolveFaqData(attrs: Record<string, unknown>, dbContext: any): FaqBlockData {
  const appearance: FaqAppearance = {
  ...
  const links: FaqLink[] = Array.isArray(attrs.links) ? attrs.links : [];
