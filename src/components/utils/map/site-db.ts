import type { MapBlockData, MapAppearance, MapLink, MapItem, MapMedia, MapIcon } from "./types";

function text(value: unknown): string | undefined {
  return typeof value === "string" && value.trim() ? value : undefined;
}

function media(value: unknown): MapMedia | undefined {
  if (typeof value === "string") return { src: value, alt: "" };
  if (typeof value === "object" && value !== null && "src" in value) {
    return value as MapMedia;
  }
  return undefined;
}

export function resolveMapData(attrs: Record<string, unknown>, dbContext: any): MapBlockData {
  ...
  const links: MapLink[] = Array.isArray(attrs.links) ? attrs.links : [];
  const items: MapItem[] = Array.isArray(attrs.items) ? attrs.items : [];
